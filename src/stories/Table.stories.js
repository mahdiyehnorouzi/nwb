...
export default {
  ...
  argTypes: {
    ...
    sortMultiple: { control: 'boolean' },
  },
};

function attrs(a) {
  const { striped, selectable, fixedHeader, sortMultiple } = a;
  return [
    striped ? 'striped' : '',
...
function mountTable(innerHtml, setData) {
  const wrap = document.createElement('div');
  wrap.innerHTML = innerHtml.trim();
  queueMicrotask(() => {
    const table = wrap.querySelector('n-table');
    if (table && typeof setData === 'function') {
      table.data = setData();
    }
  });
  return wrap;
...
const BasicTemplate = ({ striped, selectable, fixedHeader, sortMultiple }) =>
  ...
  <div style="max-height: 400px; overflow: auto;">
    <n-table ${attrs({ striped, selectable, fixedHeader, sortMultiple })}>
